...
export async function POST(request: NextRequest) {
	try {
		...
		for (const sheetName of workbook.SheetNames) {
			...
			// Определяем тип недели по названию листа
			let defaultWeekType = 'both'
			const sheetNameLower = sheetName.toLowerCase()
			if (
				sheetNameLower.includes('числ') ||
				sheetNameLower.includes('числитель')
			) {
				defaultWeekType = 'numerator'
			} else if (
				sheetNameLower.includes('знам') ||
				sheetNameLower.includes('знаменатель')
			) {
				defaultWeekType = 'denominator'
			}

			console.log(`Sheet "${sheetName}" -> weekType: ${defaultWeekType}`)
		...
		if (allEntries.length > 0) {
			console.log(`Saving ${allEntries.length} entries to database`)
			...
			for (const numEntry of numeratorEntries) {
				const hasDenominator = denominatorEntries.some(
					denEntry =>
						...
						denEntry.dayOfWeek === numEntry.dayOfWeek &&
...
function parseSheet(
	...
	if (groupRowIndex === -1) {
		console.log('No group row found in sheet')
		return entries
	}
		...
		if (
			...
			if (samePairNum && /^\d+$/.test(samePairNum)) {
				...
				if (teacherRow) {
					...
					if (!nextFirstCell && (!nextPairNum || !/^\d+$/.test(nextPairNum))) {
				...
				for (const groupInfo of groups) {
					...
					// Определяем тип недели (может быть переопределен из названия предмета)
					let weekType = defaultWeekType
